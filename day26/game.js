...
const state = {
  ...
  // 有沒有按過「出征」。畫面上寫著「請先輸入名字」,
  // 但之前沒有任何程式擋住 —— 說明說必填、實作沒擋,這種不一致本身就是 bug
  ready: false,
};
...
function showResult(result) {
  ...
  resultBox.hidden = false;        // hidden 是 HTML 屬性,拿掉它元素才會出現
}
...
function attack() {
  ...
  // 還沒出征就不能打。說明寫了「請先輸入名字」,程式就該真的擋住 ——
  // 而且要說明原因,不是讓玩家對著沒反應的按鈕困惑
  if (!state.ready) {
    addLog("請先輸入勇者名字,並按下「出征!」", "log-warning");
    nameInput.focus();             // 順手把游標移到該填的地方
    return;
  }
...
function reset() {
  ...
  // ready 要重置:再戰一次必須重新按出征。
  // 一開始我覺得「名字都確認過了,不用再按一次」,但那樣會有陷阱 ——
  // 玩家改了輸入框卻沒按出征,新名字不會被讀取;而一攻擊輸入框就鎖住,
  // 他會以為自己改了名字,而且再也改不掉。
  // 強制重新出征雖然多一個步驟,但保證「畫面上的名字 = 實際使用的名字」
  state.ready = false;
