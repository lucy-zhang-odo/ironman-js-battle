...
// ---------------------------------------------------------------------
// 🥚 彩蛋:名字自訂樣式(Day 15 埋的伏筆)
//
// Day 15 我發現玩家可以在名字裡塞 HTML 標籤,讓它在戰鬥訊息裡被解析 ——
// 那是一個真的 XSS 漏洞。當時我用 escapeHtml 把它堵起來。
//
// 但「能改自己名字的樣式」其實是個有趣的功能。所以這裡用論壇的老方法:
//   第一步:把所有真的 HTML 轉義掉(攻擊到這裡就全死了)
//   第二步:只把「我自己定義的標記」還原成標籤
//
// 這叫白名單:不是「禁止我想得到的壞東西」,而是「只允許我列出的東西」。
// 玩家輸入 <img src=x onerror="..."> 會原樣顯示成文字;
// 輸入 [red]小明[/red] 才會變成紅色的小明。
// ---------------------------------------------------------------------
const NAME_TAGS = {
  "[b]": "<b>",       "[/b]": "</b>",
  "[i]": "<i>",       "[/i]": "</i>",
  "[red]":   '<span style="color:#c0392b">',
  "[blue]":  '<span style="color:#2a6496">',
  "[green]": '<span style="color:#1a7a1a">',
  "[/red]": "</span>", "[/blue]": "</span>", "[/green]": "</span>",
};

function styleName(raw) {
  let s = escapeHtml(raw);                       // 先全部轉義,攻擊在這裡就結束了
  for (const mark in NAME_TAGS) {
    s = s.replaceAll(mark, NAME_TAGS[mark]);     // 只還原白名單裡的標記
  }
  return s;
}

// 算長度時不該把標記算進去 —— 玩家的名字是「小明」,不是「[red]小明[/red]」
function plainName(raw) {
  let s = raw;
  for (const mark in NAME_TAGS) {
    s = s.replaceAll(mark, "");
  }
...
function renderFighter(fighter, nameEl, textEl, fillEl) {
  ...
  nameEl.innerHTML = styleName(fighter.name);
...
function showResult(result) {
  ...
  if (result === "win") {
    ...
    resultDetail.innerHTML = `${styleName(state.hero.name)} 撐了 ${state.turn} 回合`;
  }
...
function endGame(result) {
  ...
  if (result === "win") {
    ...
    addLog(
      isNewRecord
        ? `<b>${styleName(state.enemy.name)}</b> 被擊敗了!${state.turn} 回合 —— 新紀錄!`
        : `<b>${styleName(state.enemy.name)}</b> 被擊敗了!(共 ${state.turn} 回合)`,
      "result-win"
    );
  } else {
    state.hero.hp = 0;
    ...
    diceBtn.disabled = true;
    addLog(`<b>${styleName(state.hero.name)}</b> 倒下了…… 撐了 ${state.turn} 回合`, "result-lose");
  }
...
function faceDesc(f) {
  ...
  if (f.kind === "heal") {
    return `${styleName(state[f.target].name)}回復 ${f.value} 點血`;
  }
  const who = styleName(state[f.target].name);
...
function enemyCounterAttack(heroDisplayName) {
  ...
  pendingTimer = setTimeout(() => {
    ...
    addLog(`<b>${styleName(state.enemy.name)}</b> 反擊 <span style="color:#c0392b">${enemyDmg}</span> 點!`);
...
function blockedByGuards() {
  if (isAnimating) {
    addLog(`${styleName(state.hero.name)}請不要偷襲!`, "log-warning");
    return true;
  }
  if (!state.ready) {
...
function startGame() {
  ...
  // 長度算的是「去掉樣式標記之後」的實際名字
  const plain = plainName(name);
  if (plain.length > 10) {
    addLog(`名字太長了(${plain.length} 字),請縮短到 10 字以內`, "log-warning");
    return;
  }

  // 🥚 名字對上了就切到魔王視角,否則維持正常模式(用去掉標記的名字比對)
  const isBoss = BOSS_NAMES.includes(plain);
  state.bossMode = isBoss;
  applySetup(isBoss ? BOSS_SETUP : NORMAL_SETUP);
  document.body.classList.toggle("boss-mode", isBoss);

  // 兩種模式都要保留玩家輸入的原文 ——
  // 逆位模式如果用 BOSS_SETUP 裡寫死的「魔王」覆蓋掉,
  // 玩家的樣式標記就被丟掉了(標題用的是 setup,所以不受影響)
  state.hero.name = name;

  if (isBoss) {
    addLog("……有人在呼喚黑暗。", "log-ghost");
    addLog(`<b>${styleName(name)}</b>睜開了眼睛 —— 勇者來討伐你了`, "result-lose");
  } else {
    addLog(`勇者 <b>${styleName(state.hero.name)}</b> 出征!`);
  }
...
function attack() {
  ...
  if (state.isOver) {
    ...
    addLog(`${styleName(state.hero.name)}請不要對${styleName(state.enemy.name)}屍體動手!(第 ${state.whipCount} 下)`);
    return;
  }
  ...
  addLog(`<b>${styleName(state.hero.name)}</b> 造成 <span style="color:#c0392b">${heroDmg}</span> 點傷害!`, "", true);
...
function rollDice() {
  ...
  addLog(`<b>${styleName(state.hero.name)}</b> 擲出了骰子……`, "", true);
  applyDice(face);
